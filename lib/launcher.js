...
module.exports = function() {
  ...
  var start = function(config, servers, cb) {
    var stat;
    ...
    if (config === nscaleRoot + '/config/config.json' && (!fs.existsSync(config)) ) {
      ...
      initProcess.on('exit', _start(config, servers, cb));
    }
    else {
      _start(config, servers, cb);
    }
  };
  ...
  var _start = function(config, servers, cb) {
    ...
    */
    function checkServerStatus(server, cb) {
      ...
      if (fs.existsSync(pidFile)) {
        var pid = Number(fs.readFileSync(pidFile));
        var port = _getServerConfig(config, server).port || 3223;
        var host = 'localhost';
  ...
  var _getServerConfig = function(config, server) {
