    ...
    function onNextServer(server, cb) {
      serverStatus(server, function(err, status) {
        ...
        if (!status.running && status.listening) {
          msg = '\nattempted to start ' + server + ' on port ' + status.port + ' but another service was detected'
                + '\n- open ' + config + '\n- change the ' + server.replace('nscale-', '') + ' port number\n- run `nscale start` to try again\n';
          cb(Error(msg));
        }
        else if (!(status.running || status.listening)) {
