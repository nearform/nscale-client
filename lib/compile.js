...
module.exports = function(dir, cb) {
  var cloneOrPull = function(cdef, cb) {
    ...
    if (fs.existsSync(repoPath)) {
      ...
      if (fs.existsSync(path.join(repoPath, '.git'))) {
        ...
        system(['git', 'pull'], { cwd: repoPath }, function(err) {
          if (err) {
  ...
  var fetchRepo = function(cdef, cb) {
    ...
    cloneOrPull(cdef, function(err, repoPath) {
      if (err) { return cb(err); }
      exec('git rev-parse HEAD', { cwd: repoPath }, function(err, stdout, stderr) {
        if (err) { return cb(err); }
