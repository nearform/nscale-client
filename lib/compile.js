...
module.exports = function(dir, cb) {
  ...
  var fetchRepo = function(cdef, cb) {
    ...
    if (!repositoryUrl) { return cb(null, cdef); }
  ...
  compiler.compile(dir, 'local', function(err, system) {
    if (err) { return cb(err); }
    async.map(system.containerDefinitions, fetchRepo, function(err, newCdefs) {
      if (err) { return cb(err); }
      system.containerDefinitions = newCdefs;
      fs.writeFile(path.join(dir, 'system.json'), JSON.stringify(system, null, 2), cb);
    });
  });
};
